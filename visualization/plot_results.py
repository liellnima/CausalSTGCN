...
def plot_losses(train_losses, valid_losses, metric_name, train_params):
    ...
    # plot baseline loss
    plt.axhline(y = baseline_valid_loss, c='g', linestyle="--", label="DCDI baseline")

    # stuff around the plot
    plt.xlabel("epochs")
    plt.legend()
    plt.title("{}: Experiment ID {}, Batchsize {}, Learning Rate {}".format(
        metric_name,
        train_params["exp_id"],
    ...
    plt.savefig("./checkpoint/exp_{}/losses_{}_{}_{}_{}.png".format(
        train_params["exp_id"],
        metric_name,
        train_params["epochs"],
        train_params["batch_size"],
